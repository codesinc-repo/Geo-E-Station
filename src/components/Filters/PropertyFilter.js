...
const PropertyFilter = ({ onSubmit, onFilterResults }) => {
  ...
  const fetchProvinces = async () => {
    try {
      const response = await axios.get(
        "https://apis.geoestate.ai/api/filter-properties/provincias"
      );
  ...
  const fetchPropertyTypes = async () => {
    try {
      const response = await axios.get(
        "https://apis.geoestate.ai/api/filter-properties/tipos-inmueble"
      );
  ...
  const fetchIslands = async (provinceName) => {
    try {
      const response = await axios.get(
        `https://apis.geoestate.ai/api/filter-properties/islands?provincia=${provinceName}`
      );
  ...
  const fetchMunicipalities = async (province, island) => {
    try {
      const response = await axios.get(
        `https://apis.geoestate.ai/api/filter-properties/municipios?provincia=${province}&&island=${island}`
      );
  ...
  // Update your fetchZones function (around line 128):

  // Fetch zones for a selected municipality
  const fetchZones = async (municipio) => {
    try {
      const response = await axios.get(
        `https://apis.geoestate.ai/api/filter-properties/zonas?municipio=${municipio}`
      );

      // Format the data to ensure it has consistent property names
  ...
  const searchLocations = async (query) => {
    ...
    try {
      const response = await axios.get(
        `https://apis.geoestate.ai/api/filter-properties/location-suggestions?query=${query}`
      );
  ...
  const buildApiUrl = () => {
    ...
    if (formData.selectedIsland && formData.selectedIsland !== "Select") {
      const selectedProvinceData = provinces.find(
        (p) => p.name === formData.selectedProvince
      );
      const selectedIslandData = selectedProvinceData?.items.find(
        (i) => i.id.toString() === formData.selectedIsland.toString()
      );

      if (selectedIslandData) {
        params.append("Islands", selectedIslandData.name);
      }
    } else if (formData.islands.length > 0) {
      // Keep backward compatibility with checkbox selection
      const selectedIslands = provinces
        .flatMap((p) => p.items)
        .filter((item) => formData.islands.includes(item.id))
        .map((item) => item.name);
    ...
    if (
      formData.selectedMunicipality &&
      formData.selectedMunicipality !== "Select"
    ) {
      const selectedMunicipalityData = municipalities.find(
        (m) => m.Id.toString() === formData.selectedMunicipality.toString()
      );

      if (selectedMunicipalityData) {
        params.append("Municipality", selectedMunicipalityData.Name);
      }
    }

    // Add zone if selected
    if (formData.selectedZone && formData.selectedZone !== "Select") {
      const selectedZoneData = zones.find(
        (z) => (z.Id || z.id).toString() === formData.selectedZone.toString()
      );

      if (selectedZoneData) {
        params.append("Zone", selectedZoneData.Name || selectedZoneData.name);
      }
    }
    ...
    params.append(
      "RentalType",
      formData.transactionType === "sale" ? "Sale" : "Rent"
    );
    ...
    const amenities = formData.bathrooms.filter(
      (item) =>
        item !== "Swimming pool" &&
        [
          "Elevator",
  ...
  const handleIslandDropdownChange = (e) => {
    ...
    // If a valid island is selected, fetch municipalities
    if (islandId !== "Select") {
      const selectedProvinceData = provinces.find(
        (p) => p.name === formData.selectedProvince
      );
      const selectedIslandData = selectedProvinceData?.items.find(
        (i) => i.id.toString() === islandId.toString()
      );

      if (selectedIslandData) {
        fetchMunicipalities(selectedProvinceData.name, selectedIslandData.name);
      }
    }
  };
  ...
  const handleMunicipalityChange = (e) => {
    ...
    // If a valid municipality is selected, fetch zones
    if (municipalityId !== "Select") {
      fetchZones(municipalityId);
    }
  };
  ...
  const handleReset = () => {
    setFormData({
      ...
      statusFilters: [],
      selectedProvince: "Select",
      selectedIsland: null, // Add this line
      selectedMunicipality: null, // Add this line
      selectedZone: null, // Add this line
      islands: [],
      locationSearch: "",
      locations: [],
      sortBy: "date-desc",
  ...
  const locationGroups = [
    {
      category: "ALARÓ",
      items: [{ name: "Alaró", isSelected: false }],
    },
    {
      category: "ALCUDIA",
      items: [
    ...
    {
      category: "ARIANY",
      items: [{ name: "Ariany", isSelected: false }],
    },
  ];
  ...
  const statusFilters = [
    {
      id: 1,
      label: "New listings on the market (last 48 hours)",
      checked: false,
    },
  ...
  return (
    <form
      className="property-filter-container filter-con"
    ...
    >
      ...
      <div className="mt-3">
        ...
        {formData.selectedProvince !== "Select" && (
          <div className="mt-3">
            <label className="text-muted section-label">Select island</label>
            <select
              className="form-control"
              value={formData.selectedIsland || "Select"}
              onChange={handleIslandDropdownChange}
            >
              <option value="Select">Select</option>
              {provinces
        ...
        {formData.selectedIsland && formData.selectedIsland !== "Select" && (
          <div className="mt-3">
            ...
            <select
              className="form-control"
              value={formData.selectedMunicipality || "Select"}
        ...
        {formData.selectedMunicipality &&
          formData.selectedMunicipality !== "Select" && (
            <div className="mt-3">
              <label className="text-muted section-label">Select zone</label>
              <select
                className="form-control"
                value={formData.selectedZone || "Select"}
                onChange={handleZoneChange}
              >
                <option value="Select">Select</option>
                {zones.map((zone) => (
                  <option key={zone.Id || zone.id} value={zone.Id || zone.id}>
                    {zone.Name || zone.name}
                  </option>
                ))}
              </select>
                  ...
                  <div className="location-group-items">
                    {group.items.map((item, itemIndex) => (
                      ...
                      >
                        ...
                        <span
                          style={{
                            color: formData.selectedLocations.includes(
                              item.name
                            )
                              ? "#f90"
                              : "#333",
        ...
        <div className="btn-group transbtngroup">
          <button
            ...
            className={`btn transbtn ${
              formData.transactionType === "sale" ? "active" : ""
            }`}
          ...
          <button
            type="button"
            className={`btn transbtn ${
              formData.transactionType === "rent" ? "active" : ""
            }`}
          ...
          <div className="p-3 rounded bg-light d-flex gap-5 justify-content-center mt-3">
            <div className="d-flex column-gap-1 align-items-center">
              <input
                ...
                onChange={() =>
                  handleCheckboxChange("rentFurnished", !formData.rentFurnished)
                }
              />
              <label htmlFor="furnished" className="fw-normal mb-1">
                Furnished
              </label>
            </div>
            <div className="d-flex column-gap-1 align-items-center">
              <input
                ...
                onChange={() =>
                  handleCheckboxChange(
                    "rentUnfurnished",
                    !formData.rentUnfurnished
                  )
                }
              />
              <label htmlFor="unfurnished" className="fw-normal mb-1">
                Unfurnished
              </label>
            </div>
          </div>
        ...
        <div className="d-flex gap-2 flex-wrap">
          {bedroomOptions.map((option, index) => (
            <button
              type="button"
              key={index}
              className={`btn bedroom-btn btn-outline-primary ${
                formData.bedrooms.includes(option) ? "active" : ""
              }`}
              onClick={() =>
                handleCheckboxChange(
                  "bedrooms",
        ...
        <div className="d-flex gap-2 flex-wrap">
          {bathroomOptions.map((option, index) => (
            <button
              ...
              key={index}
              className={`btn bedroom-btn btn-outline-primary ${
                formData.bathrooms.includes(option) ? "active" : ""
              }`}
              onClick={() =>
                handleCheckboxChange(
                  "bathrooms",
          ...
          <div className="d-flex align-items-center">
            <input
              ...
              onChange={() =>
                handleCheckboxChange(
                  "touristLicense",
                  formData.touristLicense === true ? null : true
                )
              }
            />
            <label className="fw-light">Yes</label>
          </div>
          <div className="d-flex align-items-center">
            <input
              type="checkbox"
              ...
              onChange={() =>
                handleCheckboxChange(
                  "touristLicense",
                  formData.touristLicense === false ? null : false
                )
              }
            />
            <label className="fw-light">No</label>
          </div>
        </div>
      </div>
      <div className="filter-section d-flex flex-wrap">
          ...
          <div className="d-flex align-items-center">
            <input
              ...
              onChange={() =>
                handleCheckboxChange(
                  "accessibility",
                  formData.accessibility === true ? null : true
                )
              }
            />
            <label className="fw-light">Yes</label>
          ...
          <div className="d-flex align-items-center">
            <input
              ...
              onChange={() =>
                handleCheckboxChange(
                  "accessibility",
                  formData.accessibility === false ? null : false
                )
              }
            />
            <label className="fw-light">No</label>
